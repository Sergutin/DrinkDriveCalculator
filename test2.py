...
def process_time(drinks_num, st_drinks_location, drink):
    ...
    time = drinks_num * list(st_drinks_home.values())[drink - 1]
    p_time = str(round(time, 1))
    time_home = datetime.now() + timedelta(hours=time)
    date = time_home.strftime("%H:%M %Y-%m-%d")
    print(f'The alcohol should have left your system in: {p_time}h at {date}')
    print("\n")
